...
<div align="center">
  ...
  <p>
    把 AI 生成项目的 README、使用手册、交付说明、发布站点和反馈闭环，收拢进一个可编辑、可审核、可发布的文档工作台。
  </p>
...
编辑器内选中文本即可唤起 AI 浮层；右侧文档 Insight 则通过规则分析展示术语一致性、摘要、健康度和关联建议。
...
- 文档资产库支持上传图片、GIF、视频和常见附件，编辑器可一键插入 Markdown 引用，也支持剪贴板粘贴图片/附件后自动上传并插入引用，构建静态站时自动复制到 `assets/`。
...
| [AI 能力确认](docs/AI_CAPABILITY_REVIEW.md) | 区分 LLM 能力、规则分析和后续规划 |
...
- 前端测试覆盖还不完整。
- 静态站 SEO、站内搜索、媒体资产库筛选、重命名和批量管理还需要继续增强。
- 多人协作、审批流、实时协同还在路线图中。

## 路线图摘要

- v0.2：静态站阅读体验、搜索、SEO、截图级示例站。
- v0.3：媒体资产库筛选/重命名/批量管理、公式和 Mermaid 完整渲染。
